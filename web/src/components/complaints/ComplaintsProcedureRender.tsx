...
// Pure renderer for a practice's complaints procedure. No data fetching,
// no editor logic — just structured prose driven by the JSON config and
// a small slice of public practice contact info.
//
// Two consumers (kept identical visually):
//   - Booking app:  live preview inside the Settings editor.
//   - Marketing:    public /complaints page (mirrored copy of this file
//                   under marketing/src/components/).
//
// National regulator contacts are hardcoded HERE — they're the same for
// every UK practice, so centralising them avoids each practice having to
// keep five external phone numbers + addresses correct. If any of these
// change, update them in both copies of this file.

export interface PracticePublicContact {
  /** Practice trading name shown in headings + closing references. */
  name: string;
  /** Single-line address components — only the non-empty ones render. */
...
interface ComplaintsProcedureRenderProps {
  ...
  /**
   * When true, hide the "Last reviewed" line. Useful in the booking-app
   * preview where the date may not yet be saved.
   */
  hideReviewedDate?: boolean;
}

// National regulator contacts — verified 2026-01.
// Update both copies of this file (web/ + marketing/) when these change.
const REGULATORS = {
  gdcPrivateComplaints: {
    name: "Dental Complaints Service",
    blurb:
      "An independent service run by the GDC for complaints about private dental treatment.",
    phone: "020 8253 0800",
  ...
  gdcMain: {
    ...
    blurb:
      "The UK regulator for dental professionals. Use this route if your complaint is about a clinician's conduct or fitness to practise.",
    phone: "020 7167 6000",
  ...
  cqc: {
    ...
    blurb:
      "The independent regulator for health and social care in England. The CQC can take action against providers that aren't meeting their standards.",
    phone: "03000 616 161",
    url: "https://www.cqc.org.uk",
  },
  ombudsman: {
    name: "Parliamentary and Health Service Ombudsman",
    blurb: "The final stage for unresolved NHS complaints in England.",
    phone: "0345 015 4033",
...
export function ComplaintsProcedureRender({
  ...
  return (
    <article className="prose prose-lg max-w-none prose-headings:font-display prose-headings:text-ink prose-a:text-primary">
      ...
      {!hideReviewedDate && data.last_reviewed_at && (
