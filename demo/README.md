...
> **Sandbox, not the live template.** This folder was seeded from `marketing/`
> as a safe place to experiment with new demo-site designs (Slider-Revolution-
> inspired animated hero, bento grids, before/after gallery, etc.) without
> risking the deployable `marketing/` template. Once a design is proven here,
> port the winning pieces back into `marketing/`.
>
> - Dev: `npm run dev:demo` (from repo root) → http://localhost:5177
> - Deliberately excluded from the aggregate `npm run build` / `npm run dev`.
> - `demo/dist/` is gitignored (experimental output is not tracked).
