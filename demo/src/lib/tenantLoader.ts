...
import { practice as staticConfig } from "@/config/practice.config";
...
// Build a synthetic tenant from the static marketing config so the demo can
// render without a matching DB practice row. DEV-only (see caller).
function devFallbackTenant(hostname: string): MarketingTenant {
  return {
    hostname,
    practice: {
      ...
      slug: "demo",
      status: "ACTIVE",
      country: staticConfig.address.country,
      timezone: "Europe/London",
      marketing_site_enabled: true,
      booking_app_enabled: true,
      ico_registration_number: null,
      cqc_provider_id: null,
      cqc_rating: null,
      cqc_rating_date: null,
      complaints_procedure: null,
      principal_dentist_name: null,
      principal_dentist_gdc_number: null,
    },
  };
}
...
export async function loadTenantConfig(): Promise<BootResult> {
  ...
  if (!row) {
    // Sandbox convenience: in DEV, a hostname that maps to no practice row
    // (e.g. localhost) renders the full template from the *static* config
    // instead of the "coming soon" gate — so designers can preview the demo
    // standalone. DEV-only; production builds still gate on a real tenant.
    if (import.meta.env.DEV) {
      return { kind: "found", tenant: devFallbackTenant(hostname) };
    }
